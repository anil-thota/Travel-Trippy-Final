...
(function () {
		  ...
		  .then(data => {
			if (data.banners && data.banners.length > 0) {
			  ...
			  data.banners.forEach((banner, index) => {
				...
				console.log(`Banner ${index + 1} Image URL:`, banner.image);
				...
				carouselItem.innerHTML = `
				  <img src="${banner.image}" class="d-block w-100" alt="${banner.heading}">
					...
					bannersHtml += `
						<div class="col-md-4 wow fadeInDown col-9" data-wow-delay=".2s">
							<ul class="list-marked-2 box-categories-list">
								<li>
									<a href="#">
										<img src="${banner.image}" alt="${banner.heading}" style="width: 400px; height: 300px; object-fit: cover;"/>
									</a>
									<h5 class="box-categories-title">${banner.subHeading}</h5>
								</li>
							</ul>
						</div>
					`;
	...
	function updateAboutSection() {
		...
		fetch(`https://purple-backend-362176970036.asia-south1.run.app/properties/${PROJECTID}/about`)
			...
			.then(data => {
				...
				// Optionally, update image dimensions if necessary
				aboutImageElement.style.width = '400px';
	...
	function updateAddressDetails() {
		...
		fetch(`https://purple-backend-362176970036.asia-south1.run.app/properties/${PROJECTID}/product`)
		  ...
		  .then(data => {
			if (data.allProducts && data.allProducts.length > 0) {
			  // Log the travel details for debugging
			  console.log('Travel details:', data);
	  
			  // Iterate over the products and update the corresponding DOM elements
			  data.allProducts.forEach((product, index) => {
				// Assuming the index corresponds to the order of the HTML structure
				var productArticle = document.querySelectorAll('.product-big')[index];
	  
				if (productArticle) {
				  // Update title
				  var destinationTitle = productArticle.querySelector('.product-big-title a');
				  destinationTitle.textContent = product.title;
	  
				  // Update description
				  var destinationDescription = productArticle.querySelector('.product-big-text');
				  destinationDescription.textContent = product.description;
	  
				  // Update price
				  var travelPrice = productArticle.querySelector('.product-big-price');
				  travelPrice.textContent = `$${product.price}`;
	  
				  // Update image
				  var destinationImage = productArticle.querySelector('.product-big-figure img');
				  destinationImage.src = product.images[0];
				  destinationImage.alt = product.title;
				} else {
				  console.error(`No article found for travel details at index ${index}`);
				}
			  });
			} else {
			  console.error('No travel products found in the response');
			}
		  })
		  .catch(error => {
			console.error('Error fetching the travel details:', error);
		  });
	  }
	  ...
	  async function fetchAndDisplayTeamMembers() {
		try {
		  const response = await fetch(`https://purple-backend-362176970036.asia-south1.run.app/properties/${PROJECTID}/product`);
		  const data = await response.json();
		  const teamContainer = document.getElementById('team-container');
	  
		  // Filter out team members based on the category name
		  const teamMembers = data.allProducts.filter(product => product.category.name === 'TEAM');
				...
				<article class="team-modern">
				  <div class="team-modern-header">
					<a class="team-modern-figure" href="#">
					  <img class="img-circles" src="${member.images[0]}" alt="" width="118" height="118"/>
					</a>
				  ...
				  <div class="team-modern-caption">
					<h6 class="team-modern-name"><a href="#">${member.title}</a></h6>
					<p class="team-modern-status">${member.description}</p>
...
function updateBannerImages() {
	fetch(`https://purple-backend-362176970036.asia-south1.run.app/properties/${PROJECTID}/product`)
	  ...
	  .then(data => {
		...
		// Filter the data where category name is 'Banner'
		const bannerData = allProducts.filter(item => item.category.name === 'BANNER');
  
		// Select the parallax container where the background image will be applied
		const parallaxContainer = document.querySelector('.parallax-container');
  
		// If there's banner data, use the first banner as the background image
		if (bannerData.length > 0) {
		  const banner = bannerData[0]; 
		  console.log("Banner data:", banner);
  
		  // Check if the banner has an images array and at least one image
		  if (banner.images && banner.images.length > 0) {
			// Set background image using the first image in the array
			parallaxContainer.style.backgroundImage = `url(${banner.images[0]})`; 
			parallaxContainer.style.backgroundSize = 'cover'; // Ensure image covers the container
			parallaxContainer.style.backgroundPosition = 'center'; // Center the image
			parallaxContainer.style.backgroundRepeat = 'no-repeat'; // Prevent image from repeating
		  } else {
			console.warn('No images found for this banner.');
		  }
		} else {
		...
		fetch(apiUrl)
			...
			.then(data => {
				...
				if (aboutUsBanner && aboutUsBanner.image) {
					...
					const aboutUsSection = document.querySelector('.box-position.breadcrumbs-01');
					...
					if (aboutUsSection) {
						...
						console.error("The section '.box-position.breadcrumbs-01' was not found.");
					}
				} else {
		...
		fetch(apiUrl)
			...
			.then(data => {
				...
				if (aboutUsBanner && aboutUsBanner.image) {
					...
					// Select the section where the background image needs to be applied
					const aboutUsSection = document.querySelector('.box-position.breadcrumbs-03');
